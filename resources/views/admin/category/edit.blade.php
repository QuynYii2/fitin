        ...
        <section class="section dashboard">
            <div class="bg-white p-4">
                <form action="{{url("admin/category/update",$data->id)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row mt-3">
                        <div class="col-3">Danh mục cha :</div>
                        <div class="col-8">
                            <select name="parent_id" class="form-control">
                                <option value="0">Làm danh mục cha</option>
                                @foreach($category as $categorys)
                                    <option value="{{$categorys->id}}" @if($data->parent_id == $categorys->id) selected @endif>{{$categorys->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mt-3">
                        <div class="col-3">Là key seach :</div>
                        <div class="col-8">
                            <div class="form-check form-switch">
                                <input class="form-check-input" name="is_key" @if($data->is_key == 1) checked @endif type="checkbox"
                                       id="flexSwitchCheckChecked">
                                <label class="form-check-label" for="flexSwitchCheckChecked">Làm key search </label>
                            </div>
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-3">Hiện ở trang home :</div>
                        <div class="col-8">
                            <div class="form-check form-switch">
                                <input class="form-check-input" name="is_home" @if($data->is_home == 1) checked @endif type="checkbox"
                                       id="flexSwitchCheckChecked">
